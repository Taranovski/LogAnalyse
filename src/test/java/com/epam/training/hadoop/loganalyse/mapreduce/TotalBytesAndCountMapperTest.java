...
public class TotalBytesAndCountMapperTest {

    private static final Mapper MAPPER = new TotalBytesAndCountMapper();
//    private Reducer reducer = new WordCountReducer();
    private static MapDriver<LongWritable, Text, Text, TotalBytesAndCountWritable> mapDriver;

    public TotalBytesAndCountMapperTest() {
    }
    ...
    @Test
    public void testMyMapperWithoutErrors() {
        try {
            List<Pair<Text, TotalBytesAndCountWritable>> expectedOutput = new LinkedList<>();
            expectedOutput.add(new Pair<>(new Text("ip1"), new TotalBytesAndCountWritable(1L, 40028L)));
            expectedOutput.add(new Pair<>(new Text("ip1"), new TotalBytesAndCountWritable(1L, 56928L)));
    ...
    @Test
    public void testMyMapperWithErrors() {
        try {
            List<Pair<Text, TotalBytesAndCountWritable>> expectedOutput = new LinkedList<>();
            expectedOutput.add(new Pair<>(new Text("ip1"), new TotalBytesAndCountWritable(1L, 40028L)));
            expectedOutput.add(new Pair<>(new Text("ip2"), new TotalBytesAndCountWritable(1L, 318L)));
